...
class WordVecOpt(object):
    ...
    def loadModelfromFile(self, modelFilePath):
        ...
        '''
        self.model = Word2Vec.load(modelFilePath)
        return self.model

    def initTrainWord2VecModel(self, corpusFilePath):

        corpusFile = open(corpusFilePath, u'r', encoding='utf-8')
        print('training model from singleFile!')
        model = Word2Vec(LineSentence(corpusFile), size=self._size, window=self._window,
                         min_count=self._minCount, workers=self._workers)
        corpusFile.close()
        # model.init_sims()
    ...
    def updateW2VModelUnit(self, corpusSingleFilePath):
        '''
        ...
        #     trainedWordCount = self.model.train(LineSentence(corpusSingleFilePath))
        #     print('update model, update words num is: ' + str(trainedWordCount))
        # elif fileType == u'file':
        corpusSingleFile = open(corpusSingleFilePath, u'r', encoding='utf-8')
        # ss = ["111 22 33 ","44 55 66"]
        self.model.train(LineSentence(corpusSingleFile)
                         # self.model.train(ss,
                         #                  ,total_examples=self.model.corpus_count
                         #                  ,epochs=self.model.iter
                         )
        trainedWordCount = self.model.train(LineSentence(corpusSingleFile))
    ...
    def save(self):
        ...
        self.model.save(self.modelPath + "1")
        print(1)
